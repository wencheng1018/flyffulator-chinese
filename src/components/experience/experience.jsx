...
function Experience() {
    ...
    function getKillsPerLevel() {
        ...
        for (let i = 0; i < monsters.length; ++i) {
            ...
            data.push({
                value: kills,
                name: monsters[i].level + "  " + (monsters[i].name[shortCode] ?? monsters[i].name.en)
            });
        }
                            ...
                            <div className="teammate" key={i}>
                                <div className="row" style={{ justifyContent: "space-between", alignItems: "start" }}>
                                    <div className="column">
                                        ...
                                        {
                                            i == 0 ?
                                                <>{t("level")} {Context.player.level}</>
                                                :
                                                <div className="row">
                                                    <NumberInput min={1} max={165} label={t("level")} onChange={(v) => setTeammateValue(i, "level", v)} value={p.level} />
                                                </div>
                                        }
                                    </div>
                                    {
